...
def getDistance(ranges, angles):
    ...
    if(len(ranges) > 50):
        center1_min_index = np.where(math.radians(160) < angles)[0][0]#megkapjuk a látószögben legközelebb lévő pontot(gifen a kék range)
        center1_max_index = np.where(math.radians(179.9) < angles)[0][0]
        tmp1 = np.arange(center1_min_index, center1_max_index, 1)
        center2_min_index = np.where(math.radians(-179.9) < angles)[0][0] #megkapjuk a látószögben legközelebb lévő pontot(gifen a kék range)
...
def getFarthestNeighbours(filteredranges):
    p1, p2 = 0, 0
    maxdiff = 0
    for i in range(len(filteredranges) - 1):
        if abs(filteredranges[i][1] - filteredranges[i+1][1]) > maxdiff:
            p1 = filteredranges[i][0]
